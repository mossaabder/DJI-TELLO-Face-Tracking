...
def trackFace( info, w, h, pid_x, pid_y, pError_x, pError_y):
    up = 0
    area = info[1]
    x , y =  info[0]
    fb = 0
    ...
    error_x = x - w//2
    error_y = y - h//2
    """calcule la vitesse de commande PID pour l'ange de l'acet"""
    speed_x = pid_x[0]*error_x + pid_x[1]*(error_x - pError_x)
    speed_x = int(np.clip(speed_x,-100,100))
    '''calcule la vitesse de commande up and down'''
    ...
    if y == 0:
        speed_y = 0
        error_y = 0
    print("speed_yaw",speed_x)
    print("speed up and down", speed_y)
    print("fb",fb)
...
while True:
    ...
    pError = trackFace(info,w, h, pid_x, pid_y, pError_x, pError_y)
    print("Area", info[1])
